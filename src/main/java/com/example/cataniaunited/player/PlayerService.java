...
public class PlayerService {
    ...
    public void updatePlayerResources (String playerId, HashMap<TileType, Integer> remainingResources) throws GameException {
        Player player = getPlayerById(playerId);
        if (player == null){
            logger.errorf("Update of resources failed, no player found: playerId = %s", playerId);
            throw new GameException("Player with id %s not found", playerId);
        }
        player.setResources(remainingResources);
    }
